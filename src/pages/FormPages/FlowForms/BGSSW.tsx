...
export default () => {
    ...
    useEffect(() => {
        if (formContext.task.loading === false && StringUtils.isNullOrEmpty(formContext.watch('archType'))) {
            onChangeArchType(archTypes[0])
        }
    }, [formContext.task.loading, formContext.watch('archType')])

    //公文类型发生变化
    const onChangeArchType = (archType: any) => {
        if (formContext.task.value?.canEditForm === true) {
            formContext?.setValue('archType', archType)
            if (archType === "办件") {
                dealLeveOnChange(banjianEmergencyLevels[3].name)
            } else {
                dealLeveOnChange(yuejianEmergencyLevels[0].name)
            }
        }
    }
    ...
    //办理登记发生变化
    const dealLeveOnChange = (emergencyLevel: string) => {
        if (formContext.task.value?.canEditForm === true) {
            formContext.setValue('emergencyLevel', emergencyLevel)
    ...
    //办结时间限制发生变化
    const onChangeDays = (days: string) => {
        if (formContext.task.value?.canEditForm === true) {
