...
const key = 'OfficeGoodsManagement'
...
export default () => {
    ...
    }, [reloadData.value, name, userDisplayName, department, userName])
    ...
    const columns: ColumnsType<IItem> = [
        ...
        },
        {
            ...
            render: (item: IItem) => <Space>
                <Button onClick={() => descItem(item)}>领用</Button>
                <Button onClick={() => addItem(item)}>归还</Button>
    ...
    return <MainLayout>
        <Breadcrumb>
            ...
            <Breadcrumb.Item>办公用品管理</Breadcrumb.Item>
        </Breadcrumb>
        <hr />
        <Space>
            <Button onClick={dlg.Toggle}>办公用品登记入库</Button>
        ...
        <MyDialog open={dlg.value} title="办公用品登记入库" onClose={dlg.Toggle}>
            <Form>
                <Form.Item label="办公用品名称">
...
const StatDiglog = ({ visual }: { visual: IuseToggleResult }) => {
    ...
    return <>
        <MyDialog title="办公用品统计分析" open={visual.value} onClose={visual.Toggle}>
