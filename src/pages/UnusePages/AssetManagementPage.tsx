...
export default () => {
    ...
    return <MainLayout>
        <Breadcrumb>
            ...
            <Breadcrumb.Item>固定资产管理</Breadcrumb.Item>
        </Breadcrumb>
        <hr />
        <Space>
        ...
        <MyDialog open={dlg.value} title="新建固定资产" onClose={dlg.Toggle}>
            <Form>
                <Form.Item label="固定资产名称">
...
const StatDiglog = ({ visual }: { visual: IuseToggleResult }) => {
    ...
    return <>
        <MyDialog title="固定资产统计分析" open={visual.value} onClose={visual.Toggle}>
